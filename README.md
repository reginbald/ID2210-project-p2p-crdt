...
This component listens to the Logoot Channel for Logoot_Do,Logoot_Undo and Lo- goot_Redo messages, and responds with Logoot_Done, Logoot_Doc and Logoot_Patch messages.
...
When the CORB_Deliver message contains Logoot_Undo(patchId: UUID, _) it re- trieves the patch from the history buffer, decreases the degree of the patch and if the degree is equal to 0 it execute the inverse of the operations in the patch.

When the CORB_Deliver message contains Logoot_Redo(patchId: UUID, _) it re- trieves the patch from the history buffer, increases the degree of the patch and if the degree is equal to 1 it execute the operations in the patch.

### No-Waiting Causal Broadcast Component
Implements no-waiting causal broadcast algorithm. The component listens in on the CausalOrderReliableBroadcast channel for CORB_Broadcast messages and on the Reli- able Broadcast channel for RB_Deliver messages.
...
The component listens to the ReliableBroadcast Channel for RB_Broadcast messages and wraps them in a GBEB_Broadcas message before sending them to the BestEffor- Broadcast Channel.

The component receives GBEB_Deliver messages from the BestEfforBroadcast Chan- nel and delivers them to the component above(No-Waiting Causal Broadcast Compo- nent) in an RB_Deliver message as well as broadcasting the same messages back to the BestEfforBroadcast Channel.
...
The following properties are verified using three different scenarios in the PingTest- Client.
